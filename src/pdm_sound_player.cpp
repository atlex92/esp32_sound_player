...
PdmSoundPlayer::PdmSoundPlayer(const gpio_num_t clk_io, const gpio_num_t data_io)
: clk_gpio_{clk_io}, dout_gpio_{data_io} {
...
void PdmSoundPlayer::play(const SoundSample* samples, const size_t size) {
  ...
  while (bytes_sent != bytes_to_sent) {
    ...
    const size_t length{(bytes_to_sent - bytes_sent) >= TX_CHUNK_LENGTH_IN_BYTES(freq_)
                            ? TX_CHUNK_LENGTH_IN_BYTES(freq_)
                            : (bytes_to_sent - bytes_sent)};
    ESP_ERROR_CHECK(i2s_channel_write(tx_chan_handler_, (void*) (data_to_send + bytes_sent), length, &bytes_written,
                                      portMAX_DELAY));
