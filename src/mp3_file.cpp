...
SampleChunk Mp3File::nextSampleChunk() const {
    const size_t chunk_size{((data_size_ - data_offset_) >= FILE_CHUNK_SIZE) ? FILE_CHUNK_SIZE : (data_size_ - data_offset_)};
    const int samples_decoded{mp3dec_decode_frame(&mp3d_, data_ + data_offset_, chunk_size, samples_.get(), &frame_info_)};
